          ...
          <div className="relative mb-8">
            <div className="relative">
              <textarea
                ...
                rows={1}
                className="w-full px-6 py-4 bg-secondary border border-border rounded-2xl text-foreground placeholder-muted-foreground resize-none focus:outline-none focus:ring-2 focus:ring-primary focus:border-primary transition-all duration-200 min-h-[60px] max-h-32"
                style={{
                  height: 'auto',
                  minHeight: '60px'
                }}
              ...
              <div className="absolute right-4 top-1/2 transform -translate-y-1/2 flex items-center space-x-2">
                ...
                <button
                  ...
                  className="p-2 bg-primary hover:bg-primary/90 disabled:bg-secondary rounded-lg transition-all duration-200 disabled:cursor-not-allowed"
                  title="Send message"
                >
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
            {[
              ...
              <button
                ...
                className="p-4 bg-secondary hover:bg-accent border border-border rounded-xl transition-all duration-200 text-left group shadow-sm hover:shadow-md"
                type="button"
              >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {[
              ...
              <button
                ...
                className="p-4 bg-secondary hover:bg-accent border border-border rounded-xl text-left transition-all duration-200 group"
                type="button"
              >
                <div className="text-foreground font-medium group-hover:text-muted-foreground transition-colors">
                  {suggestion}
                </div>
              </button>
            ))}
      ...
      <div className="flex items-center justify-between p-6 border-b border-border bg-background">
        ...
        <div className="flex items-center space-x-3">
          <button
            ...
            className="p-3 rounded-2xl bg-secondary hover:bg-accent transition-all duration-200 border border-border"
          ...
          {isLoading && (
            <button
              ...
              className="p-3 rounded-2xl bg-primary hover:bg-primary/90 transition-all duration-200 border"
            ...
            >
              <div className={`flex max-w-[85%] ${message.role === 'User' ? 'flex-row-reverse' : 'flex-row'} items-start space-x-4`}>
                <div className={`flex-shrink-0 w-10 h-10 rounded-2xl flex items-center justify-center shadow-lg ${
                  message.role === 'User' 
                    ? 'bg-primary ml-4' 
                    : 'bg-background border-2 border-border mr-4'
                }`}>
                  {message.role === 'User' ? (
          ...
          {isLoading && (
            <div className="flex justify-start">
              <div className="flex max-w-[85%] items-start space-x-4">
                <div className="flex-shrink-0 w-10 h-10 rounded-2xl bg-background border-2 border-border mr-4 flex items-center justify-center shadow-lg">
                  <Bot className="w-5 h-5 text-foreground" />
                </div>
                <div className="bg-background border border-border rounded-3xl px-6 py-4 shadow-lg">
          ...
          <div className="flex items-end space-x-4">
            <div className="flex-1 relative">
              <textarea
                ...
                rows={1}
                className="w-full px-6 py-4 bg-secondary border border-border rounded-3xl text-foreground placeholder-muted-foreground resize-none focus:outline-none focus:ring-2 focus:ring-primary focus:border-primary transition-all duration-200 min-h-[56px] max-h-32"
                style={{
                  height: 'auto',
                  minHeight: '56px'
                }}
            ...
            <button
              onClick={handleSubmit}
              disabled={!inputValue.trim() || isLoading}
              className="w-14 h-14 bg-primary hover:bg-primary/90 disabled:bg-secondary rounded-2xl flex items-center justify-center transition-all duration-200 disabled:cursor-not-allowed shadow-lg hover:shadow-xl"
            >
              <Send className="w-5 h-5 text-primary-foreground" />
            </button>
          </div>
